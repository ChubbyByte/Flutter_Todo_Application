...
class TodoScopedModel extends Model {
  ...
  void fetchTasks() async {
    try {
      final String apiEndPoint =
          "http://192.168.1.16/todo_db/api/tables/getData_task.php";
      final Uri url = Uri.parse(apiEndPoint);
      final http.Response response = await http.get(url);
  ...
  Future<Map<String, dynamic>> addTask(
      Map<String, dynamic> myDataToSend) async {
    _isLoading = true;
    notifyListeners();

    // Map<String, dynamic> responseFromPHP = {};
    ...
    try {
      final String apiEndPoint =
          "http://192.168.1.16/todo_db/api/tables/addTask.php";
      final Uri url = Uri.parse(apiEndPoint);
      ...
      // print('the responseDecoded: $responseDecoded');

      // responseDecoded.forEach((data) {
      //   print('the data: $data');
      // });
      // responseFromPHP['success'] = response.body.;

      _isLoading = false;
      notifyListeners();

      return Future.value(responseDecoded); //checking state is done? or NOT!
    } catch (e) {
      Map<String, dynamic> failMap = {
        "error": false,
        "message": "Adding task falied!, check youe internet connection",
      };
      _isLoading = false;
      notifyListeners();
      // print('there is an error: $e');

      return Future.value(failMap);

      print('The error: $e');
    }
  }

  Future<bool> addTodo(Map<String, dynamic> myDataToSend) async {
    _isLoading = true;
    notifyListeners();
    ...
    try {
      final String apiEndPoint =
          "http://192.168.1.16/todo_db/api/tables/addTodo.php";
      final Uri url = Uri.parse(apiEndPoint);
      final http.Response response = await http.post(url, body: myDataToSend);
